...
(define-constant N u4)             ;; query count        (must equal the query-counter length below + PARAMS)
(define-constant L u3)             ;; FRI commitment count (gear 6e, rate 1/2): 1 FIRST-layer root (the
...
(define-constant DOMAIN_SIZE u16)  ;; query-index modulus (DOCUMENTED DEFAULT; the real LDE coset size + the
...
(define-constant QUERY_COUNTER (list u0 u1 u2 u3))  ;; length N; the values are ignored, only the LENGTH matters
...
(define-read-only (derive-challenges
    ...
    (nonce (buff 8)))
  (let ((a (contract-call? .transcript squeeze-qm31
             (contract-call? .transcript absorb-root (contract-call? .transcript transcript-init ctx) trace-root))))
  (let ((z (contract-call? .transcript squeeze-qm31
             (contract-call? .transcript absorb-root (get state a) comp-root))))
  (let ((s-open (contract-call? .transcript absorb-qm31
                  (contract-call? .transcript absorb-qm31
                    (contract-call? .transcript absorb-qm31
                      (contract-call? .transcript absorb-qm31
                        (contract-call? .transcript absorb-qm31
                          (contract-call? .transcript absorb-qm31
                            (contract-call? .transcript absorb-qm31 (get state z) (contract-call? .commit qm31-enc16 t-z))
                            (contract-call? .commit qm31-enc16 t-gz))
                          (contract-call? .commit qm31-enc16 t-g2z))
                        (contract-call? .commit qm31-enc16 c0-z))
                      (contract-call? .commit qm31-enc16 c1-z))
                    (contract-call? .commit qm31-enc16 c2-z))
                  (contract-call? .commit qm31-enc16 c3-z))))
  (let ((g (contract-call? .transcript squeeze-qm31 s-open)))
  (let ((fr (fold fri-beta-step fri-roots { state: (get state g), betas: (list) })))
  (let ((s-fin (contract-call? .transcript absorb-qm31 (get state fr) (contract-call? .commit qm31-enc16 final))))
  (let ((pass (contract-call? .transcript pow-ok s-fin nonce POW_THRESHOLD)))
  (let ((s-n (contract-call? .transcript absorb-nonce s-fin nonce)))
