...
(define-constant DOMAIN_SIZE u16)       ;; toy LDE coset size (= 2^L); the real size is a human question (QUERY-5)
(define-constant HALF u8)               ;; DOMAIN_SIZE / 2 (the half-coset size)
(define-constant CM_POW_BOUND u8)       ;; >= HALF: H has order 8, so the factored exponent is always < 8
(define-constant CM_STEPS (list u0 u1 u2)) ;; 3 unrolled square-and-multiply bits (exponent < 8)

;; canonic-coset constants (CM31 points), verified against Stwo's generator
(define-constant OFF { re: u1179735656, im: u1241207368 }) ;; G^(2^26), order 32
(define-constant H   { re: u32768,      im: u2147450879 }) ;; G^(2^28), order 8
...
(define-constant BITREV4 (list u0 u8 u4 u12 u2 u10 u6 u14 u1 u9 u5 u13 u3 u11 u7 u15))
...
(define-private (cm (a { re: uint, im: uint }) (b { re: uint, im: uint }))
  (contract-call? .qm31 cm-mul (get re a) (get im a) (get re b) (get im b)))
