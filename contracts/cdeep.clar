...
(define-constant SX u32768)         ;; own copy of the trace step (KAT-pinned == cair's)
...
(define-read-only (line-coeffs
  ...
  (begin
    ...
    (let ((araw (contract-call? .qm31 qm31-sub (conj-u v) v))
          (craw (contract-call? .qm31 qm31-sub (conj-u zy) zy)))
      { a: (contract-call? .qm31 qm31-mul w araw),
        b: (contract-call? .qm31 qm31-mul w
             (contract-call? .qm31 qm31-sub
               (contract-call? .qm31 qm31-mul v craw)
               (contract-call? .qm31 qm31-mul araw zy))),
        c: (contract-call? .qm31 qm31-mul w craw) })))
...
(define-read-only (denom-inv
    ...
    (zy { c0: uint, c1: uint, c2: uint, c3: uint })
    (px uint) (py uint))
  (let ((t1 (contract-call? .qm31 cm-mul
              (mod (- (+ (get c0 zx) P) px) P) (get c1 zx)
              (get c2 zy) (get c3 zy)))
        (t2 (contract-call? .qm31 cm-mul
...
(define-read-only (quot-term
  ...
  (contract-call? .qm31 qm31-sub
    (contract-call? .qm31 qm31-mul (get c lc) f)
    (contract-call? .qm31 qm31-add
      (contract-call? .qm31 qm31-mul-m31 (get a lc) py)
      (get b lc))))
...
(define-private (rot-s (zx { c0: uint, c1: uint, c2: uint, c3: uint })
  ...
  { x: (contract-call? .qm31 qm31-sub
         (contract-call? .qm31 qm31-mul-m31 zx SX)
         (contract-call? .qm31 qm31-mul-m31 zy SY)),
    y: (contract-call? .qm31 qm31-add
         (contract-call? .qm31 qm31-mul-m31 zx SY)
         (contract-call? .qm31 qm31-mul-m31 zy SX)) })
...
(define-read-only (deep-row
  ...
  (let ((g2 (contract-call? .qm31 qm31-mul gamma gamma)))
  (let ((g3 (contract-call? .qm31 qm31-mul g2 gamma)))
  (let ((g4 (contract-call? .qm31 qm31-mul g3 gamma)))
  (let ((g5 (contract-call? .qm31 qm31-mul g4 gamma)))
  (let ((g6 (contract-call? .qm31 qm31-mul g5 gamma))
        (z1 (rot-s zx zy)))
  (let ((z2 (rot-s (get x z1) (get y z1)))
        (one { c0: u1, c1: u0, c2: u0, c3: u0 }))
    (let ((num0 (contract-call? .qm31 qm31-add
      ...
      (contract-call? .qm31 qm31-add
        (contract-call? .qm31 qm31-add
          (contract-call? .qm31 qm31-mul num0 (denom-inv zx zy px py))
          (contract-call? .qm31 qm31-mul num1 (denom-inv (get x z1) (get y z1) px py)))
        (contract-call? .qm31 qm31-mul num2 (denom-inv (get x z2) (get y z2) px py)))))))))))
